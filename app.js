...
const URL_TO_TEST = "http://172.16.0.210:8383/apex/f?p=109";

const CSV_FILE_PATH = "response_times.csv";
const usersFile = "user.csv";

var responseTime1 = Date.now();
var responseTime2 = Date.now();

const timeout = 30000 * 1;
...
async function runStressTest() {
  ...
  const promises = Array.from({ length: SESSION_COUNT }, async (_, index) => {
    try {
      ...
      console.log(
        `Session ${index + 1} - User ${users[index][0]} - Response Time 1: ${
          responseTime1 / 1000
        } s`
      );
      ...
      console.log(
        `Session ${index + 1} - User ${users[index][0]} - Response Time 2: ${
          responseTime2 / 1000
        } s`
      );
  ...
  const csvContent =
    "Session,User,ResponseTime1,ResponseTime2\n" +
    results
      .map(
        (result) =>
          `${result.session},${result.user},${result['responseTime1']},${result['responseTime2']}`
      )
      .join("\n");
